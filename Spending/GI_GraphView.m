...
@implementation GI_GraphView
@synthesize week, weekdate, data;
int maxValue = 20000;
...
- (void)drawLineGraphWithContext:(CGContextRef)ctx
{
    ...
    for (int i = 0; i < 7; i++)
    {
        ...
        NSDate *currentSelectedDate = [GI_Date date].selectedDate;
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for (int i = 0; i < kNumberOfBars; i++)
    {
        if (CGRectContainsPoint(touchAreas[i], point))
        {
            ...
            [GI_Date date].selectedDate = weekdate[i];
...
- (void)drawRect:(CGRect)rect
{
    ...
    NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
    [dateFormat setDateFormat:@"yyyy-MM-dd"];
    NSString *from=[dateFormat stringFromDate:weekdate[0]];
    NSString *to=[dateFormat stringFromDate:weekdate[6]];
    
    
    
    UILabel *range = [[UILabel alloc] initWithFrame:self.frame];
    range.frame = CGRectMake(self.frame.size.width/2-73, 0, 160, 13);
    range.backgroundColor = [UIColor whiteColor];
    range.textAlignment = NSTextAlignmentCenter;
    range.text = [NSString stringWithFormat:@"%s  \u2014  %s", [from UTF8String], [to UTF8String]];
    range.font = [UIFont fontWithName:@"HelveticaNeue" size:12];
    range.textColor = [UIColor lightGrayColor];
    ...
    CGContextSelectFont(context, "HelveticaNeue", 8, kCGEncodingMacRoman);
